...
public class StopwatchCompatibilityTests
{
    private readonly ITestOutputHelper _out;

    public StopwatchCompatibilityTests(ITestOutputHelper output) => _out = output;

    [Fact]
    public void StopwatchElapsed_AssignedToTimeSpan_CompilesAgainstCompatRuntime()
    {
        var result = Convert("""
using System;
using System.Diagnostics;
...
class Sample {
    static int ReadElapsedHours(Stopwatch sw) {
...
""");
        ...
        Assert.Contains("CSharpTimeSpan ts = sw.getElapsed()", code, StringComparison.Ordinal);
        code = code.Replace("import io.github.ningpp.compat.CSharpTimeSpan;",
            "import io.github.ningpp.compat.CSharpTimeSpan;\nimport io.github.ningpp.compat.StopwatchHelper;",
        ...
        var compatDir = Path.Combine(FindRepoRoot(), "java", "csharptojava-compat", "src", "main", "java", "io", "github", "ningpp", "compat");
        var compatSources = new[]
        {
            "CSharpTimeSpan.java",
            "ObjectHolder.java",
            "StopwatchHelper.java",
