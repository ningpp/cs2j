        ...
        const string generated = """
            ...
            public class CdtTests {
                public void smallTriangulation() {
                    var cdt = new Cdt(points(), null, new ArrayList<>(Arrays.stream(new SymmetricTuple[] { new SymmetricTuple<Point>(new Point(109, 202), new Point(506, 135)), new SymmetricTuple<Point>(new Point(139, 96), new Point(452, 96)) }).collect(Collectors.toList())));
                }

                public void withCut() {
                    var cut = new SymmetricTuple[] { new SymmetricTuple<Point>(new Point(80, 80), new Point(90, 75)) };
                    var cdt = new Cdt(Arrays.asList(corners), Arrays.asList(holes), new ArrayList<>(Arrays.stream(cut).collect(Collectors.toList())));
                }
