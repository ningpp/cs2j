...
public class ControlFlowTransformer : IIRExpressionTransformer
{
    ...
    private string TransformConditional(ConditionalExpressionSyntax node, ConversionContext context)
    {
        ...
        // Drain branch-scoped pre-statements before transforming WhenTrue.
        // These are generated by object initializer extraction and must be
        // inlined into the if-else block, not hoisted to the statement level.
        context.DrainBranchScopedPreStatements();
        var trueExpr = facade.Transform(node.WhenTrue, context);
        var trueBranchPres = context.DrainBranchScopedPreStatements();
        ...
        var falseBranchPres = context.DrainBranchScopedPreStatements();
        ...
        if (trueBranchPres.Count > 0 || falseBranchPres.Count > 0)
        {
