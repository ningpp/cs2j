...
public class InvocationExpressionTransformer : IIRExpressionTransformer
{
        ...
        ExpressionTransformerFacade facade)
    {
        ...
        // System.Random.Next overloads are handled by TypeMappings.json (Next → next)
        // and the CSharpRandom compat class, which already replicates C# edge-case
        // semantics (maxValue<=0 → 0, min>=max → min). No special handling needed.
