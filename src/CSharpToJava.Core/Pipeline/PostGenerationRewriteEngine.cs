...
public static class PostGenerationRewriteEngine
{
    public static void ApplyCompatibilityRewrites(List<ConversionResult> results)
    {
        foreach (var r in results)
        {
            ...
            code = code.Replace("Arrays.stream(initialLayering).map(i -> i + 1).max(Comparator.naturalOrder()).orElseThrow()", "Arrays.stream(initialLayering).map(i -> i + 1).max().orElseThrow()", StringComparison.Ordinal);
            ...
            code = code.Replace("Arrays.stream(layering).max(Comparator.naturalOrder()).orElseThrow()", "Arrays.stream(layering).max().orElseThrow()", StringComparison.Ordinal);
            ...
            code = code.Replace("Arrays.stream(layerList.get(i)).map(j -> nodes.get(j).getBoundingBox().getTop()).max(Comparator.naturalOrder()).orElseThrow()", "Arrays.stream(layerList.get(i)).mapToDouble(j -> nodes.get(j).getBoundingBox().getTop()).max().orElseThrow()", StringComparison.Ordinal);
            code = code.Replace("Arrays.stream(layerList.get(i + 1)).map(j -> nodes.get(j).getBoundingBox().getBottom()).min(Comparator.naturalOrder()).orElseThrow()", "Arrays.stream(layerList.get(i + 1)).mapToDouble(j -> nodes.get(j).getBoundingBox().getBottom()).min().orElseThrow()", StringComparison.Ordinal);
            code = code.Replace("Arrays.stream(layerList.get(i)).map(j -> nodes.get(j).getBoundingBox().getRight()).max(Comparator.naturalOrder()).orElseThrow()", "Arrays.stream(layerList.get(i)).mapToDouble(j -> nodes.get(j).getBoundingBox().getRight()).max().orElseThrow()", StringComparison.Ordinal);
            code = code.Replace("Arrays.stream(layerList.get(i + 1)).map(j -> nodes.get(j).getBoundingBox().getLeft()).min(Comparator.naturalOrder()).orElseThrow()", "Arrays.stream(layerList.get(i + 1)).mapToDouble(j -> nodes.get(j).getBoundingBox().getLeft()).min().orElseThrow()", StringComparison.Ordinal);
            ...
            code = code.Replace(".sorted(Comparator.comparing(e -> e.getLength())", ".sorted(Comparator.comparing((Microsoft.Msagl.Core.Layout.Edge e) -> e.getLength())", StringComparison.Ordinal);
            ...
            code = code.Replace("new Polyline(ConvexHull.calculateConvexHull(Stream.concat(StreamSupport.stream(poly.spliterator(), false), Arrays.stream(stickingPointsArray).boxed()).collect(Collectors.toList())))", "new Polyline(ConvexHull.calculateConvexHull(new ArrayList<Point>(Stream.concat(StreamSupport.stream(poly.spliterator(), false), Arrays.stream(stickingPointsArray).boxed()).collect(Collectors.toList()))))", StringComparison.Ordinal);
