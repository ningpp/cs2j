...
- ✅ 转换器输出结构化 Java IR（MethodTransformer/PropertyTransformer/ConstructorTransformer 使用 StructuredBody）
- ✅ IR 层后处理基础设施（ConversionPipeline/ProjectConversionPipeline.AddIRRewriter + JavaSyntaxRewriter 链）
- ⬜ 具体 IR 重写器实现（迁移 PostGen 补丁到 IR 层）
...
- ✅ CLI 层集成 WorkspacePlan + MavenPomGenerator 替代现有 POM 生成（消除 ~280 行硬编码 XML 模板）
...
| 指标 | 当前基线 | 阶段 0 结果 | 阶段 1 进度 | 阶段 3 进度 | 阶段 4 进度 | 阶段 4+ FQN 清理 | CLI 集成 | StructuredBody + IR 重写 | 最终目标 |
|------|---------|-----------|-----------|-----------|-----------|----------------|---------|------------------------|---------|
| `ProjectConversionPipeline.cs` 行数 | 4,697 | 145 ✅ | 145 | 145 | 145 | 224 | 224 | 224 | ≤ 300 |
| PostGenerationRewriteEngine 补丁数 | 586 | 586 | 501（-85） | ~499 | 440 | 497 | 497 | 497 | ≤ 50 |
| `ConversionContext.cs` 行数 | 1,149 | 1,149 | 1,149 | 221 ✅ | 221 | 221 | 221 | 221 | ≤ 300 |
| Java AST 行数 | 682 | 682 | 682 | 2,029 ✅ | 2,029 | 2,029 | 2,029 | 2,029 | ≥ 2,000 |
| 回归测试通过/失败 | 461/15 | 461/15 | 461/15 | 474/0 ✅ | 499/0 ✅ | 504/0 ✅ | 504/0 ✅ | 504/0 ✅ | 全部通过 |
